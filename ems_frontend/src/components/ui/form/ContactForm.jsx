...
const ContactForm = ({ title, id }) => {
    ...
    useEffect(() => {
        ...
        const fetchContact = async () => {
            try {
                ...
                setForm({
                    name: res.data.contacts.name,
                    email: res.data.contacts.email,
                    phone_no: res.data.contacts.phone_no,
                    pan_no: res.data.contacts.pan_no,
                    department_id: res.data.contacts.department_id,
                    location_id: res.data.contacts.location_id,
                    contact_type: res.data.contacts.contact_type
                });
            } catch (error) {
                console.error(error);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (id) {
               const res = await axiosInstance.put(`/contacts/${id}`, form);
               if(res){
                    ...
                    <div className="flex justify-center items-center">
                        <form onSubmit={handleSubmit} className="w-3/4">
                            ...
                            <div className="flex gap-3 items-center mb-6 ">
                                ...
                                <select
                                    name="contact_type"
                                    className="w-1/3 p-2 rounded-md border border-[#D1D1D1]"
