...
const UnsettledReimbursement = () => {
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            try {
                const res = await axiosInstance.get('/reimbursements/unsettled');
    ...
    const selectAll = () => {
        setSelectedTransaction(prev =>
            prev.length === reimbursements.length
                ? []
                : reimbursements.map(t => t.id)
        );
    };
    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectedTransaction(reimbursements.map(t => t.id));
        } else {
    ...
    const handleLogs = async () => {
        try {
            const res = await axiosInstance.post('/reimbursement/update', { ids: selectedTransaction });

            console.log(res.data.reimbursements)
        } catch (error) {
            console.error(error)
                                ...
                                <th className="py-3 px-2">
                                    <input type="checkbox" onClick={selectAll} onChange={handleSelectAll}
                                        checked={
                                            ...
                                            selectedTransaction.length === reimbursements.length
                                        } />
                                </th>
...
export default UnsettledReimbursement